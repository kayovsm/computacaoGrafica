...
GLuint cam = 0;
GLfloat fAspect = 1;
GLfloat directionFront = 0;
GLfloat accelerationCar = 0;
...
void displayPersp(void) {
  ...
  glPushMatrix();
  {
    ...
    glPushMatrix();
    {
	  ...
	  glRotatef(directionFront, 0, 1, 0); // direção das rodas da frente
	  glRotatef(accelerationCar, 0, 0, 1); // aceleração das rodas da frente
    ...
    glPushMatrix();
    {
	  ...
	  glRotatef(accelerationCar, 0, 0, 1); // aceleração das rodas de trás
...
void init() {
  ...
  // habilita a sobreamento
  glEnable(GL_LIGHTING);
  glEnable(GL_LIGHT0);

  // posição da luz pontual em cima do carro
...
void specialKeys(int key, int x, int y) {
  switch (key) {
    case GLUT_KEY_LEFT:
      angleY -= 10;
	//   directionFront -= 10;
	//   directionFront = (directionFront < -30) ? -30 : directionFront;
      break;
    case GLUT_KEY_RIGHT:
      angleY += 10;
	//   directionFront += 10;
